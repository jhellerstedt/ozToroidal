...
Function EqualiseYintensityAcrossX(data, ch_av,smth) //this function linearly correct the internsity of the data in the Y direction resulting in  data[p][lastY] = data[p][0]. Uses a linear correction
wave data //the data which will be corrected
variable ch_av //how many rows at the start and end of data are used to produce the ratio function
variable smth

	variable ysize = dimsize(data,1)-1
	make/o/n = (dimsize(data,1)) ywave
	make/o/n = (dimsize(data,0)) xwave1=0, xwave2=0, ratiowave=0
	variable i, j

		for(i=0;i<ch_av;i+=1)
			xwave1[]+=data[p][i]
			xwave2[]+=data[p][dimsize(data,1)-1-i]
		endfor
	
	ratiowave = xwave1/xwave2
	//Smooth/F smth, ratiowave
	
	for(i=0;i<dimsize(data,0);i+=1)
		ywave = data[i][p] //take each column from data
		ywave[] = ywave[p]*(1+ (ratiowave[i]-1)*(p/ysize))   ///normalise that column by the ratio, preserving the initial value and end value being /=ratio, with linear interpolation of the division in between
		data[i][] = ywave[q]
	endfor
...
Function TrackSurfState(data, left,right, smooth1, smooth2) //this function tracks the position of the surface state as it weaves about it a non normally oriented sample's exp
wave data
variable left , right, smooth1, smooth2
...
make/o/n=(dimsize(data,1)) centrewave
	...
	for(i=0;i<dimsize(data,1);i+=1)
		xline = data[p][i]
		Findpeak/B=(smooth1)/Q/R=(left,right) xline
		pos1=V_peakloc
		Findpeak/B=20/Q/R=(right,left) xline
		pos2 = V_peakloc
		Centrewave[i] = (pos1+pos2)/2
	endfor
	smooth/B smooth2, Centrewave
